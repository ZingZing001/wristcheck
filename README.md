...
The command exits `0` when approved and `2` when denied or timed out. In Copilot sessions, the intended flow is the normal Copilot CLI approval prompt first. If there is no CLI response after 10 seconds and the agent can still act, it sends WristCheck as a fallback; future approvals go to WristCheck immediately until you approve or deny from CLI again. WristCheck approve/deny responses are high-importance and authoritative when they arrive first. A deny from any channel is terminal and must not be overridden by a later approve.
